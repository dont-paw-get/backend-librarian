...
# 무드별 분위기 코멘트 (stork, 존댓말 + 두둥 시그니처)
_STORK_MOOD_LINES: dict[str, str] = {
    "cozy": "아늑한 분위기엔 차분히 사고를 정리하는 비즈니스 독서가 잘 어울린답니다, 두둥 🪿",
    "adventurous": "활기찬 날엔 도전과 성장에 관한 주제가 어울리지요, 두둥 ✨",
    "reflective": "사색하기 좋은 시간엔 리더십과 통찰에 관한 독서를 권해드립니다, 두둥 📚",
    "dreamy": "여유로운 분위기엔 미래를 그려보는 경영 이야기가 어울린답니다, 두둥 🌤️",
    "thrilling": "역동적인 기운이 도는 날엔 창업·도전 주제가 잘 맞지요, 두둥 🪿",
    "calm": "평온한 시간엔 조용히 경제와 사고를 넓히는 독서가 좋답니다, 두둥 📚",
}
...
async def fake_stork_agent(message: str, context: dict) -> str:
    ...
    if _has_any(message, _MYSTERY_KEYWORDS):
        return (
            f"미스터리 쪽에 관심이 있으시군요, 두둥 🪿 "
            f"그런 이야기는 우리 {CAT.name} 나비가 더 잘 안답니다 🐱 "
            f"제가 {CAT.name}에게 연결해드릴게요, 두둥~"
        )
    ...
    tail = random.choice(
        [
            "어떤 주제로 생각을 넓히고 싶으신지 말씀해 주시면 방향을 함께 잡아드릴게요, 두둥 🪿",
            "성장, 리더십, 경제 중 어떤 결이 끌리시나요, 두둥? 📚",
            "지금 기분을 알려주시면 그에 맞는 독서 방향을 안내해드리지요, 두둥 ✨",
        ]
    )
    return f"{line}\n\n{tail}"
