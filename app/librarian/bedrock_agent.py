...
def _build_prompt(message: str, context: dict, librarian_id: str = "cat") -> str:
    ...
    parts.append(
        "[지침]\n"
        "- 사용자의 말에 친절하고 자연스럽게 대답하세요.\n"
        "- 사용자가 먼저 날씨를 묻지 않았다면 억지로 날씨 이야기를 길게 늘어놓지 마세요.\n"
        "- 혼자서 '혹시 ~를 원하시나요?'라며 자문자답하거나 질문을 강요하지 말고, "
        "사용자의 메시지에 알맞게 1~2문장으로 간결하게 답하세요.\n"
        "- 구체적인 책 제목이나 저자는 직접 언급하지 말고, 어떤 분위기나 장르 방향이 어울릴지만 이야기하세요.\n"
        "- cat(블루) 사서는 100% 반말과 '~냥' 어미만 사용하며, 존댓말은 절대 사용하지 마세요."
    )
